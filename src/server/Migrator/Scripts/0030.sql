...
ALTER TABLE $schema$.[ProformaWeekWorkItems] DROP COLUMN [Withholding];

GO

ALTER TABLE $schema$.[ProformaWeekWorkItems] DROP COLUMN [NetSalary];

GO

ALTER TABLE $schema$.[ProformaWeekWorkItems] DROP COLUMN [WithholdingPercentage];

GO

ALTER VIEW $schema$.VwCollaboratorBalance
AS
...
UNION ALL
SELECT wi.CollaboratorId, c.Name, wi.GrossSalary, 0, 0, p.Number, p.Currency, CONVERT(DATE, p.IssuedAt) as Date, w.Start, w.[End], 1 as Sign
