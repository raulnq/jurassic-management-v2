...
public class HttpDriver : Driver
{
    ...
    public Task<(HttpStatusCode StatusCode, TResponse? Response, Microsoft.AspNetCore.Mvc.ProblemDetails? Error)> Post<TResponse>(string requestUri, Stream stream, string fileName, string contentType)
where TResponse : class
    {
        var client = CreateAuthenticatedClient();

        return client.Post<TResponse>(requestUri, stream, fileName, contentType);
    }
    ...
    public Task<(HttpStatusCode StatusCode, Microsoft.AspNetCore.Mvc.ProblemDetails? Error)> Post<TRequest>(string requestUri, TRequest request)
    {
        ...
        return client.Post(requestUri, request);
    }
    ...
    public Task<(HttpStatusCode StatusCode, Microsoft.AspNetCore.Mvc.ProblemDetails? Error)> Put<TRequest>(string requestUri, TRequest request)
    {
        ...
        return client.Put(requestUri, request);
    }
