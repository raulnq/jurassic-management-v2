...
public class EditWorkItemTests : BaseTest
{
    [Fact]
    public async Task edit_should_be_ok()
    {
        ...
        await _appDsl.Proformas.WorkItemShouldHaveRightAmounts(result!.ProformaId, 1, collaborator!.CollaboratorId, 300m, 9m);

        await _appDsl.Proformas.WeekShouldHaveRightAmounts(result!.ProformaId, 1, 150, 450);
